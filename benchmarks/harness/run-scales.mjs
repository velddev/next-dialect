...
const scales = (argv.includes('--scales') ? argv[argv.indexOf('--scales') + 1] : '1,10,100')
  .split(',')
  .map(Number)
...
for (const scale of scales) {
  ...
  if (fs.existsSync(src)) {
    const dest = path.join(here, `results-${scale}x.json`)
...
const runs = scales
  .map((s) => {
    const f = path.join(here, `results-${s}x.json`)
